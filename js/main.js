...
function makePagination() {
		...
		if (numPost <= divisor) {
			...
			if (resto == 0) {
				...
				bFin = '<button class="w3-button w3-light-grey w3-border w3-hover-red" onClick="listadoBoton('+(numIni+1)+','+numFin+')" >'+(numIni+1)+'-'+numFin+'</button>'; 
...
function listadoBoton(inicio,fin) {
	...
	function onePost(id) {
			...
			divBlog += '<div class="w3-card-4 w3-margin w3-white">'+
					...
					'<div class="w3-container">'+
						'<p>'+obj.texto_corto+'</p>'+
						//'<p class="w3-center"><img class="w3-round" src="'+obj.enlace_img+'" width=300 height=200></p>'+
						'<div class="w3-row">'+
						   '<div class="w3-col m8 s12">'+
							  '<p><button class="w3-button w3-padding-large w3-white w3-border" onclick="document.getElementById(\'expandText'+id+'\').style.display=\'block\'"><b>LEER M&Aacute;S &raquo;</b></button></p>'+
							'</div>'+
